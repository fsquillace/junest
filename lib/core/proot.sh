...
_run_env_with_proot() {
	...
	PROOT="${backend_command}" JUNEST_ENV=1 proot_cmd "${backend_args}" "$DEFAULT_SH --login" "$args"
}
...
_run_env_with_qemu() {
	...
	if [ "$JUNEST_ARCH" != "$ARCH" ]; then
		qemu_bin="qemu-$JUNEST_ARCH-static-$ARCH"
		qemu_symlink="$(mktemp)"
...
run_env_as_proot_user() {
	...
	if ! $no_copy_files; then
		...
		# This excludes /etc/mtab file so that
